-- ==========================================================
-- FixOS 4.0.0 - installer.lua
-- ЗМІНИ 4.0.0:
--   - Оновлені всі файли до 4.0.0
--   - Перевірка мінімального місця (512KB)
--   - Показ прогресу по файлах з іменами
--   - Помилки при завантаженні зберігаються у список
--   - Повне українське UI
-- ==========================================================
...
if not UI_ok then
    ...
    local function ul(s) return #tostring(s) end
    function UI.init(g) end
    function UI.truncate(s,n) s=tostring(s); if #s<=n then return s end; return s:sub(1,n-1)..">"; end
    function UI.centerText(x,y,w,str,fg,bg)
        str=tostring(str); local ox=math.max(0,math.floor((w-#str)/2))
        if bg then gpu.setBackground(bg) end; gpu.setForeground(fg); gpu.set(x+ox,y,str)
    end
    function UI.drawButton(x,y,w,h,lbl,style,en)
        style=style or "accent"; en=(en==nil) and true or en; h=h or 2
        local T=UI.Theme
        local bg=en and (style=="danger" and T.danger or style=="success" and T.success or style=="secondary" and T.surfaceAlt or T.accent) or T.surfaceInset
        local fg=(style=="secondary" and en) and T.textPrimary or T.textOnAccent
        gpu.setBackground(bg); gpu.fill(x,y,w,h," "); UI.centerText(x,y+math.floor(h/2),w,lbl,fg,bg)
    ...
    function UI.drawProgressBar(x,y,w,pct,color)
        ...
        local f=math.floor(w*pct); if f>0 then gpu.setBackground(color); gpu.fill(x,y,f,1," ") end
        local s=math.floor(pct*100).."%"; local lx=x+math.floor((w-#s)/2)
        gpu.setForeground(lx<x+f and UI.Theme.textOnAccent or UI.Theme.textSecondary)
        gpu.setBackground(lx<x+f and color or UI.Theme.progressTrack); gpu.set(lx,y,s)
    end
    function UI.hitTest(r,mx,my) return mx>=r.x and mx<r.x+r.w and my>=r.y and my<r.y+r.h end
    function UI.clearScreen(sw,sh,bg) gpu.setBackground(bg or UI.Theme.surface); gpu.fill(1,1,sw,sh," ") end
end
...
local function drawCard(cx, cy, cw, ch, title)
    ...
    for row=3,ch-2 do gpu.set(cx,cy+row,"\xE2\x94\x82"); gpu.set(cx+cw-1,cy+row,"\xE2\x94\x82") end
    gpu.set(cx,cy+ch-1,"\xE2\x94\x94"); gpu.set(cx+cw-1,cy+ch-1,"\xE2\x94\x98")
    return cy+4
end
...
local function download(url, retries)
    ...
    for attempt = 1, retries do
        local ok, h = pcall(internet.request, url, nil, {["User-Agent"]="FixOS-Installer/"..VERSION})
        if ok and h then
            local buf = {}
            ...
            while computer.uptime() < deadline do
                ...
                elseif gotData then break
                else os.sleep(0.1) end
            end
            pcall(h.close)

            local data = table.concat(buf)
            if #data > 0 and not data:lower():match("^%s*<!doctype") and not data:lower():match("^%s*<html") then
                return data
            end
        end
    ...
    for addr in component.list("filesystem") do
        ...
        if proxy then
            ...
            if tot >= MIN_SPACE and not tostring(lbl):lower():match("tmp") then
                table.insert(out, {
                    address=addr, label=lbl, size=tot, used=used,
                    free=tot-used, readOnly=ro,
                })
            end
        end
    end
...
local function formatDisk(addr)
    ...
    local function rm(path)
        ...
        if ok and isD then
            ...
            if lf then iterate(lf, function(f) rm(path..(path:match("/$") and "" or "/")..f) end) end
        end
...
local function screen_welcome()
    ...
    if not internet then
        ...
        while true do local ev,_,x,y=event.pull(); if ev=="touch" and UI.hitTest(btn,x,y) then return false end end
    end
...
local function screen_disk()
    ...
    if #disks == 0 then
        ...
        local btn=UI.drawButton(math.floor(W/2)-8,H-4,16,2,"<- Назад","secondary")
    ...
    for _, disk in ipairs(disks) do
        if dy+5 < H-6 then
            ...
            if disk.readOnly then gpu.setForeground(T.danger); gpu.set(CX+4,dy+1,"    ТІЛЬКИ ЧИТАННЯ - НЕДОСТУПНО") end
...
local function screen_install(disk)
    ...
    local function redraw(step, file, pct, msg, msgKind)
        ...
        UI.drawProgressBar(CX+2, y+3, CW-4, pct,
            pct>=1 and T.success or (pct>0.5 and T.accent or T.info))
        ...
        if msg then
            ...
            gpu.setForeground(colMap[msgKind] or T.info); gpu.setBackground(T.surface)
            gpu.set(CX+2, y+7, UI.truncate(msg,CW-4))
        end

        if #skipped>0 then
            gpu.setForeground(T.warning); gpu.setBackground(T.surface)
            gpu.set(CX+2, y+8, "Пропущено ("..#skipped.."): "..table.concat(skipped,", "):sub(1,CW-18))
        end
    end
    ...
    for _, f in ipairs(FILES) do
        ...
        redraw(
            string.format("Завантаження %d/%d...", done, total),
        ...
        if data then
            redraw(
                string.format("Запис %d/%d...", done+1, total),
...
local function main()
    if not screen_welcome() then
        UI.clearScreen(W,H); cText(math.floor(H/2),"Встановлення скасовано.",T.textPrimary); os.sleep(1); return
    end
